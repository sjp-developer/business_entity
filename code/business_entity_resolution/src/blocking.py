...
def get_lean_blocking_keys(lazy_df: pl.LazyFrame, country_str: str) -> pl.LazyFrame:
    """
    Generates vectorized blocking keys for records in country_str.
    """
    ...
    ]).select(["entity_id", "norm_name", "tok1", "tok2", "tok3", "pref5", "house_num", "postal_code"])
...
class VectorizedBlocker:
    ...
    def generate_candidates(self, l_s1: pl.LazyFrame, l_s2: pl.LazyFrame, l_s3: pl.LazyFrame, countries: List[str]) -> pl.DataFrame:
        """
        Generates lightweight candidate pairs DataFrame with schema: [s1_id, cand_id]
        """
        start_t = time.time()
        ...
        for c in countries:
            ...
            j1 = s1_k.select(["entity_id", "norm_name"]).filter(pl.col("norm_name") != "").join(
                s23_k.select(["entity_id", "norm_name"]), on="norm_name", suffix="_cand"
            ).select([pl.col("entity_id").alias("s1_id"), pl.col("entity_id_cand").alias("cand_id")])

            # 2. Tok1 + House Number
            j2 = s1_k.select(["entity_id", "tok1", "house_num"]).filter((pl.col("tok1").is_not_null()) & (pl.col("house_num").is_not_null()) & (pl.col("tok1").str.len_chars() >= 3)).join(
                s23_k.select(["entity_id", "tok1", "house_num"]), on=["tok1", "house_num"], suffix="_cand"
            ).select([pl.col("entity_id").alias("s1_id"), pl.col("entity_id_cand").alias("cand_id")])

            # 3. Tok1 + Postal Code
            j3 = s1_k.select(["entity_id", "tok1", "postal_code"]).filter((pl.col("tok1").is_not_null()) & (pl.col("postal_code").is_not_null()) & (pl.col("tok1").str.len_chars() >= 3)).join(
                s23_k.select(["entity_id", "tok1", "postal_code"]), on=["tok1", "postal_code"], suffix="_cand"
            ).select([pl.col("entity_id").alias("s1_id"), pl.col("entity_id_cand").alias("cand_id")])

            # 4. Pref5 + House Number
            j4 = s1_k.select(["entity_id", "pref5", "house_num"]).filter((pl.col("pref5").is_not_null()) & (pl.col("house_num").is_not_null()) & (pl.col("pref5").str.len_chars() >= 4)).join(
                s23_k.select(["entity_id", "pref5", "house_num"]), on=["pref5", "house_num"], suffix="_cand"
            ).select([pl.col("entity_id").alias("s1_id"), pl.col("entity_id_cand").alias("cand_id")])
            ...
            j5 = s1_capped_tok12.select(["entity_id", "tok1", "tok2"]).join(
                s23_capped_tok12.select(["entity_id", "tok1", "tok2"]), on=["tok1", "tok2"], suffix="_cand"
            ).select([pl.col("entity_id").alias("s1_id"), pl.col("entity_id_cand").alias("cand_id")])
            ...
            j6 = s1_capped_tok1.select(["entity_id", "tok1"]).join(
                s23_capped_tok1.select(["entity_id", "tok1"]), on="tok1", suffix="_cand"
            ).select([pl.col("entity_id").alias("s1_id"), pl.col("entity_id_cand").alias("cand_id")])

            cands_c = pl.concat([j1, j2, j3, j4, j5, j6]).unique()
            all_cand_dfs.append(cands_c)
            
            del s1_k, s23_k, s23_capped_tok12, s1_capped_tok12, s23_capped_tok1, s1_capped_tok1, j1, j2, j3, j4, j5, j6
            gc.collect()
